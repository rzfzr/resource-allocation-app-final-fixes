...
import compensationListExporterFields from '@/modules/compensation/compensation-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !compensationListExporterFields ||
          !compensationListExporterFields.length
        ) {
          throw new Error(
            'compensationListExporterFields is required',
          );
        }
    ...
    async doFetch(
      { commit, getters },
      { filter = null, rawFilter = null, keepPagination = false },
    ) {
      try {
        commit('FETCH_STARTED', { filter, rawFilter, keepPagination });
