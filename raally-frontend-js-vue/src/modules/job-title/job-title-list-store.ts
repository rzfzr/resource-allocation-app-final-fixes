...
import jobTitleListExporterFields from '@/modules/job-title/job-title-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !jobTitleListExporterFields ||
          !jobTitleListExporterFields.length
        ) {
          throw new Error(
            'jobTitleListExporterFields is required',
          );
        }
