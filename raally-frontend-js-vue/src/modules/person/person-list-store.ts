...
import personListExporterFields from '@/modules/person/person-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !personListExporterFields ||
          !personListExporterFields.length
        ) {
          throw new Error(
            'personListExporterFields is required',
          );
        }
