...
import projectListExporterFields from '@/modules/project/project-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !projectListExporterFields ||
          !projectListExporterFields.length
        ) {
          throw new Error(
            'projectListExporterFields is required',
          );
        }
