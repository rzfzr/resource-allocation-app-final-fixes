...
import assignmentListExporterFields from '@/modules/assignment/assignment-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !assignmentListExporterFields ||
          !assignmentListExporterFields.length
        ) {
          throw new Error(
            'assignmentListExporterFields is required',
          );
        }
